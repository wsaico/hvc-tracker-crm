...
window.switchPassengerTab = function(tab) {
    ...
    const attendedTab = document.getElementById('tab-attended');
    const searchTab = document.getElementById('tab-search');
    const todayContent = document.getElementById('today-passengers-tab');
    const attendedContent = document.getElementById('attended-passengers-tab');
    const searchContent = document.getElementById('search-passengers-tab');

    const activeClass = 'tab-button border-b-2 border-blue-600 text-blue-600 whitespace-nowrap py-4 px-1 font-medium text-sm flex items-center';
    const inactiveClass = 'tab-button border-b-2 border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 whitespace-nowrap py-4 px-1 font-medium text-sm flex items-center';

    // Resetear todos
    todayTab.className = inactiveClass;
    attendedTab.className = inactiveClass;
    searchTab.className = inactiveClass;
    todayContent.classList.add('hidden');
    attendedContent.classList.add('hidden');
    searchContent.classList.add('hidden');

    // Activar la pestaña seleccionada
    if (tab === 'today') {
        todayTab.className = activeClass;
        todayContent.classList.remove('hidden');
    } else if (tab === 'attended') {
        attendedTab.className = activeClass;
        attendedContent.classList.remove('hidden');
    } else if (tab === 'search') {
...
// Funciones de filtrado para lista de pasajeros HVC
window.filterAllPassengers = function() {
    const searchTerm = document.getElementById('filter-search')?.value.toLowerCase() || '';
    const statusFilter = document.getElementById('filter-status')?.value || 'all';
    const categoryFilter = document.getElementById('filter-category')?.value || 'all';
    const actionFilter = document.getElementById('filter-action')?.value || 'all';

    const passengerCards = document.querySelectorAll('.passenger-card');
    let visibleCount = 0;

    passengerCards.forEach(card => {
        const name = card.dataset.passengerName || '';
        const dni = card.dataset.passengerDni || '';
        const status = card.dataset.passengerStatus || '';
        const category = card.dataset.passengerCategory || '';
        const action = card.dataset.passengerAction || '';
        ...
        if (matchesSearch && matchesStatus && matchesCategory && matchesAction) {
...
window.clearAllPassengersFilters = function() {
...
const renderPassengerSearchView = async () => {
    ...
    let todayPassengers = [];
    let totalFlights = 0;
    let allPassengers = []; // TODOS los pasajeros HVC (atendidos + no atendidos)
    ...
    try {
        ...
        // Cargar todas las interacciones del aeropuerto actual
        const allInteractions = await ApiService.getAirportInteractions(state.currentAirport);
        const passengersWithInteractions = new Map();

        // Agrupar interacciones por pasajero
        allInteractions.forEach(interaction => {
            const pid = interaction.pasajero_id;
            if (!passengersWithInteractions.has(pid)) {
                passengersWithInteractions.set(pid, []);
            }
        ...
        for (const passenger of allPassengersFromDB) {
            try {
                ...
                if (hasInteractions) {
                    // PASAJERO ATENDIDO - tiene interacciones
                    const sortedInteractions = interactions.sort((a, b) => new Date(b.fecha) - new Date(a.fecha));
                    const latestInteraction = sortedInteractions[0];
                    const latestScore = latestInteraction.calificacion_medallia;

                    // Determinar acción necesaria basada en calificación
                    let action = null;
                    if (latestScore !== null && latestScore !== undefined) {
                        if (latestScore <= 6) {
                            action = {
                                type: 'urgent',
                                label: 'Requiere Recuperación Urgente',
                                icon: '🚨',
                                color: 'red'
                            };
                    ...
                    allPassengers.push({
                        ...
                        latestInteractionDate: latestInteraction.fecha,
                        latestScore: latestScore,
                        action: action,
                        interactions: sortedInteractions
                    });
                } else {
                    // PASAJERO NO ATENDIDO - sin interacciones (solo del día)
                    allPassengers.push({
                        ...passenger,
                        attended: false,
                        totalInteractions: 0,
                        latestInteractionDate: null,
                        latestScore: null,
                        action: {
                            type: 'not-attended',
                            label: 'Sin Atención Registrada',
                            icon: '❌',
                            color: 'slate'
                        },
                        interactions: []
        ...
        // Ordenar: Primero atendidos por fecha de última interacción, luego no atendidos por nombre
        allPassengers.sort((a, b) => {
            if (a.attended && b.attended) {
                return new Date(b.latestInteractionDate) - new Date(a.latestInteractionDate);
            } else if (a.attended && !b.attended) {
                return -1; // atendidos primero
            } else if (!a.attended && b.attended) {
                return 1; // atendidos primero
            } else {
                return a.nombre.localeCompare(b.nombre); // no atendidos por nombre
            }
        });

        // Obtener detalles de cada pasajero del día
        ...
        console.warn('Could not load passengers:', error);
    }
    ...
    return `
        <div class="max-w-7xl mx-auto px-3 sm:px-4 lg:px-6 py-4 sm:py-6">
            ...
            <div class="mb-4 sm:mb-6">
                <div class="border-b border-gray-200">
                    <nav class="flex space-x-8" aria-label="Tabs">
                        ...
                        <button onclick="switchPassengerTab('attended')" id="tab-attended"
                            ...
                            Todos los Pasajeros HVC
                            ${allPassengers.length > 0 ? `<span class="ml-2 bg-purple-100 text-purple-800 py-1 px-2 rounded-full text-xs font-bold">${allPassengers.length}</span>` : ''}
                        </button>
            ...
            <!-- Tab: Todos los Pasajeros HVC -->
            <div id="attended-passengers-tab" class="tab-content hidden">
                <div class="bg-white rounded-xl shadow-lg p-3 sm:p-6">
                    ...
                    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 sm:gap-0 mb-4 sm:mb-6">
                        <div>
                            <h2 class="text-xl sm:text-2xl font-bold text-gray-800 flex items-center">
                                <svg class="w-6 h-6 sm:w-7 sm:h-7 text-purple-600 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4"/>
                                </svg>
                                <span class="hidden sm:inline">Todos los Pasajeros HVC</span>
                                <span class="sm:hidden">Pasajeros HVC</span>
                            </h2>
                            <p class="text-gray-600 mt-1 text-xs sm:text-sm">
                                <span id="filtered-count">${allPassengers.length}</span> pasajero${allPassengers.length !== 1 ? 's' : ''} registrados
                            </p>
                        </div>
                        <div class="flex items-center gap-2">
                    ...
                    <!-- Filtros avanzados - Diseño responsive -->
                    <div class="bg-gradient-to-r from-purple-50 to-blue-50 rounded-lg p-3 sm:p-4 mb-4 space-y-3">
                        <!-- Búsqueda por texto -->
                        <div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                            ...
                            <div>
                                ...
                                <select id="filter-status" class="w-full px-2 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 text-xs sm:text-sm"
                                        onchange="filterAllPassengers()">
                                    <option value="all">Todos</option>
                                    <option value="attended">Atendidos</option>
                                    <option value="not-attended">No atendidos</option>
                                </select>
                            </div>

                            <!-- Filtro por categoría HVC -->
                            <div>
                                <label class="block text-xs font-medium text-gray-700 mb-1.5">Categoría</label>
                                <select id="filter-category" class="w-full px-2 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 text-xs sm:text-sm"
                                        onchange="filterAllPassengers()">
                            ...
                            <div>
                                <label class="block text-xs font-medium text-gray-700 mb-1.5">Acción necesaria</label>
                                <select id="filter-action" class="w-full px-2 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 text-xs sm:text-sm"
                                        onchange="filterAllPassengers()">
                                    <option value="all">Todas</option>
                                    <option value="urgent">🚨 Recuperación urgente</option>
                                    <option value="follow">📋 Seguimiento</option>
                                    <option value="maintain">⭐ Mantener</option>
                                    <option value="pending">⏳ Pendiente</option>
                                    <option value="not-attended">❌ Sin atención</option>
                                </select>
                            </div>

                            <!-- Botón limpiar filtros -->
                            <div class="flex items-end">
                                <button onclick="clearAllPassengersFilters()"
                                        class="w-full bg-gray-100 hover:bg-gray-200 text-gray-700 px-3 py-2 rounded-lg transition text-xs sm:text-sm font-medium flex items-center justify-center gap-2">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/>
                                    </svg>
                                    Limpiar
                    ...
                    <div id="passengers-list" class="space-y-2">
                        ...
                        `}).join('') : `
                            <div class="text-center py-16 bg-gray-50 rounded-xl border-2 border-dashed border-gray-300">
                                ...
                                <p class="text-gray-500 text-lg font-medium">No hay pasajeros atendidos aún</p>
                                <p class="text-gray-400 text-sm mt-2">Comienza a registrar interacciones para ver el historial aquí</p>
                            </div>
                        `}
                    </div>
